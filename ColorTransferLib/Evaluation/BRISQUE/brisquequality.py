...
def test_measure_BRISQUE(dis):
    ...
    dis = cv2.cvtColor(dis, cv2.COLOR_RGB2GRAY)
    ...
    # load model 
    model = svmutil.svm_load_model("Models/BRISQUE/allmodel")

    # create svm node array from python list
    x, idx = svmutil.gen_svm_nodearray(x[1:], isKernel=(model.param.kernel_type == svm.PRECOMPUTED))
