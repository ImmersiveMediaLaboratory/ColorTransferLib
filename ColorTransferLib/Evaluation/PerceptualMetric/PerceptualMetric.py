...
from ColorTransferLib.Evaluation.PerceptualMetric.saliency_models.gbvs import *
#from module.Evaluation.PerceptualMetric.saliency_models.ittikochneibur import *
...
#
# Source:
# DeepRare2019: Not implemented yet https://github.com/numediart/VisualAttention-DeepRare2019
# Used for saliency calculation: https://github.com/shreelock/gbvs
# ----------------------------------------------------------------------------------------------------------------------
# ----------------------------------------------------------------------------------------------------------------------
class PerceptualMetric:
    ...
    @staticmethod
    def apply(src, ref):
        inp = "data/images/starry-night.jpg"
        tar = "data/images/starry-night.jpg"
        res = "data/images/starry-night.jpg"

        #saliency_map_gbvs = compute_saliency(inp)
        #cv2.imwrite("/home/hpadmin/Downloads/test.png", saliency_map_gbvs)
        #exit()
        sc = PerceptualMetric.style_score(inp, tar, res)

    #compute similarity between two salienycy maps
    def saliency_similarity(input_sal1, input_sal2, target_sal1, target_sal2):
        #print(input_sal1)
        img1 = input_sal1#cv2.imread(input_sal1, 0)
        img2 = input_sal2#cv2.imread(input_sal2, 0)
        img3 = target_sal1#cv2.imread(target_sal1, 0)
        img4 = target_sal2#cv2.imread(target_sal2, 0)
        ...
        score = np.sum(diff)
        with open("module/Evaluation/PerceptualMetric/For_fit.txt", 'a') as for_fit:
    ...
    def style_score(inp, tar, res):
        ...
        with open("module/Evaluation/PerceptualMetric/metrics_summary.txt", "rb") as metrics:
            for line in metrics:
                metric = line.split()
        ...
        current_dir = "/home/hpadmin/Projects/ColorTransferLib/module/Evaluation/PerceptualMetric"

        #ssim = call(["module/Evaluation/PerceptualMetric/ssim", inp, res, '2'])
        #color_bhatt = call(["module/Evaluation/PerceptualMetric/bhattacharya", tar, res])
        #call(["module/Evaluation/PerceptualMetric/run_computeSaliency.sh", path, inp, current_dir])
        #call(["module/Evaluation/PerceptualMetric/run_computeSaliency.sh", path, res, current_dir])

        saliency_map_gbvs_inp = compute_saliency(inp)
        saliency_map_gbvs_res = compute_saliency(res)
        ...
        PerceptualMetric.saliency_similarity(saliency_map_gbvs_inp, saliency_map_gbvs_inp, saliency_map_gbvs_res, saliency_map_gbvs_res)

        #perceptual_bhatt = call(["module/Evaluation/PerceptualMetric/bhatt", tar, res])

        data_to_fit = []
        with open("module/Evaluation/PerceptualMetric/For_fit.txt", 'r') as for_fit:
            for line in for_fit:
                score = line.replace("\n", "")
